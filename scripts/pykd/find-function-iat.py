...
class AddrResolver:
    ...
    def get_iat_info(self):
        ...
        line = self.find_lines_containing(
            result.splitlines(), "Import Address Table Directory"
        )[0]
    ...
    def get_kernel32_iat_entries(self):
        result = pykd.dbgCommand(
            "dps {} {}".format(
                ...
                hex(self.image_base + self.iat_offset + self.iat_size),
            )  # end
        )
...
def main():
    ...
    parser.add_argument("module", help="address to begin search from")
    parser.add_argument(
        "func",
        choices=[
            "VirtualAllocStub",
    ...
    print(
        "[-] Using {} (couldn't find {} IAT address)".format(
            resolver.alt_entry, args.func
        )
    )
    print(
        "[+] {} ({} IAT entry)".format(
            hex(resolver.entries[resolver.alt_entry]["iat"]),
            resolver.alt_entry[9:],
        )
    )
    print(
        "[+] {} ({} resolved)".format(
            hex(resolver.entries[resolver.alt_entry]["resolved"]),
            resolver.alt_entry[9:],
        )
    )
